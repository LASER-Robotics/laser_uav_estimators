...
//
// These constants represent the physical characteristics of a real quadrotor
// UAV and are used throughout the test suite to ensure realistic simulation
// conditions. Values are based on typical mid-size racing/research drones.
// =============================================================================

/// @brief Total mass of the test drone in kilograms
/// Includes frame, motors, electronics, battery, and typical payload
const double TEST_MASS = 1.60;

/// @brief Distance from drone center to motor centerline in meters
/// Calculated from motor position matrix for torque computations
const double TEST_ARM_LENGTH = 0.258;

/// @brief Motor thrust coefficient relating motor command to thrust force
/// May require calibration based on specific motor/propeller combination
const double TEST_THRUST_COEFF = 1.0;

/// @brief Motor torque coefficient relating thrust to reactive torque
/// Used for yaw dynamics modeling in the EKF prediction step
const double TEST_TORQUE_COEFF = TEST_THRUST_COEFF * 0.59;

/// @brief Inertia matrix diagonal elements [Ixx, Iyy, Izz] in kg*m²
/// Represents the drone's resistance to angular acceleration about each axis
/// Typical values for a symmetric quadrotor with similar X and Y inertias
const Eigen::Matrix3d TEST_INERTIA =
    Eigen::Vector3d(0.4953, 0.4953, 0.3413).asDiagonal();

/// @brief Motor positions in drone body frame [x, y] coordinates in meters
/// Standard quadrotor configuration with X-pattern motor layout
/// Motor numbering: 0=Front-Right, 1=Rear-Left, 2=Front-Left, 3=Rear-Right
const Eigen::Matrix<double, 4, 2> TEST_MOTOR_POSITIONS = (Eigen::Matrix<double, 4, 2>() << 0.185, -0.18, ///< Motor 0 (Front-Right)
 ...
 * @brief Google Test fixture for StateEstimator class testing
 *
 * @details This test fixture provides a common testing environment for all
 * StateEstimator tests. It handles the setup and teardown of test instances,
 * ensuring that each test starts with a clean, properly initialized EKF.
 *
 * The fixture also provides utility methods for:
 * - Running multi-step simulations with consistent control inputs
 * - Executing complete prediction-correction cycles
 * - Formatted state output for debugging and analysis
 *
 * Key Features:
 * - Automatic EKF initialization with realistic drone parameters
 * - Hover thrust calculation based on drone mass and gravity
 * - Simulation utilities for complex flight scenarios
 * - State visualization helpers for test analysis
 *
 * @note Each test method gets a fresh EKF instance to ensure test independence
 */
...
protected:
    ...
    /// Used as baseline for all control input calculations
    const double hover_thrust_per_motor = (TEST_MASS * 9.80665) / 4.0;

    /**
     * @brief Test setup method called before each test execution
     *
     * @details Creates a new StateEstimator instance with the predefined
     * drone parameters. This ensures that each test starts with a fresh,
     * properly initialized EKF in the default state (position=0, velocity=0,
     * orientation=identity quaternion).
     *
     * The initialization includes:
     * - Drone physical parameters (mass, inertia, motor layout)
     * - Thrust and torque coefficients for dynamics modeling
     * - Default verbosity level for logging control
     */
    void SetUp() override
    {
    ...
    /**
     * @brief Test cleanup method called after each test execution
     *
     * @details Properly releases the StateEstimator instance to prevent
     * memory leaks and ensure clean test isolation. The unique_ptr
     * automatically handles the destruction, but explicit reset ensures
     * immediate cleanup.
     */
     ...
     *
     * @details This utility method simulates continuous flight with a constant
     * control input over multiple time steps. It's useful for testing:
     * - Steady-state behavior (hover, constant acceleration)
     * - Dynamic response to control inputs
     * - Filter stability over extended periods
     *
     * The method only performs prediction steps (no measurements/corrections),
     * allowing pure analysis of the dynamics model and process noise effects.
     */
     ...
     *
     * @details This method simulates realistic operation where the EKF
     * alternates between prediction (using control inputs) and correction
     * (using sensor measurements). This is the typical operating mode
     * for real-time state estimation.
     *
     * Use cases:
     * - Testing sensor fusion effectiveness
     * - Validating measurement noise handling
    ...
    /**
     * @brief Utility for formatted state vector output and analysis
     *
     * @param title Descriptive title for the state printout
     * @param state Current EKF state vector to display
     *
     * @details Provides structured, human-readable output of the drone's
     * complete state vector. The output includes:
     * - Position: [x, y, z] coordinates in meters
     * - Orientation: Quaternion [w, x, y, z] components
     * - Linear velocity: [vx, vy, vz] in m/s
     * - Angular velocity: [wx, wy, wz] in rad/s
     *
     * This is invaluable for:
     * - Debugging test failures
     * - Analyzing flight behavior
     * - Validating physical realism of results
...
TEST_F(TestStateEstimator, CorrectionDecreasesUncertainty)
{
    ...
    // Log results for test analysis and debugging
    std::cout << "\n--- Correction Convergence Test Results ---" << std::endl;
    std::cout << "Initial Trace: " << initial_trace << std::endl;
    std::cout << "Final Trace (after 100 cycles): " << final_trace << std::endl;
    std::cout << "Uncertainty Reduction: " << (initial_trace - final_trace) << std::endl;
    std::cout << "Relative Improvement: " << ((initial_trace - final_trace) / initial_trace * 100.0) << "%" << std::endl;
    std::cout << "Test Result: " << (final_trace < initial_trace ? "PASSED" : "FAILED") << std::endl;
    std::cout << "-------------------------------------------" << std::endl;
}
...
TEST_F(TestStateEstimator, ForwardMovement)
{
    ...
    Eigen::Vector4d u_forward(
        hover_thrust_per_motor - diff,  // Front left motor (reduced)
...
TEST_F(TestStateEstimator, BackwardMovement)
{
    ...
    Eigen::Vector4d u_backward(
        hover_thrust_per_motor + diff,  // Front left motor (increased)
        hover_thrust_per_motor - diff,  // Rear left motor (reduced)
...
TEST_F(TestStateEstimator, YawRotation)
{
    ...
    Eigen::Vector4d u_yaw(
        hover_thrust_per_motor + diff,  // Motor 0 (Front-Right, increased)
        hover_thrust_per_motor + diff,  // Motor 1 (Rear-Left, increased)
        hover_thrust_per_motor - diff,  // Motor 2 (Front-Left, reduced)
        hover_thrust_per_motor - diff); // Motor 3 (Rear-Right, reduced)
...
TEST_F(TestStateEstimator, TwoYawRotation)
{
    ...
    Eigen::Vector4d u_yaw(
        hover_thrust_per_motor - diff,  // Motor 0 (Front-Right, reduced)
        hover_thrust_per_motor - diff,  // Motor 1 (Rear-Left, reduced)
        hover_thrust_per_motor + diff,  // Motor 2 (Front-Left, increased)
        hover_thrust_per_motor + diff); // Motor 3 (Rear-Right, increased)
...
TEST_F(TestStateEstimator, CorrectionWithHighNoiseHasLittleEffect)
{
    ...
    std::cout << "\n--- High-Noise Measurement Test Results ---" << std::endl;
    ...
    std::cout << "Covariance reduction: " << covariance_reduction << std::endl;
    std::cout << "Relative reduction: " << (relative_reduction * 100.0) << "%" << std::endl;
    std::cout << "Expected: < 10% reduction due to high noise" << std::endl;
    std::cout << "-------------------------------------------" << std::endl;
...
TEST_F(TestStateEstimator, ExecutionTimeBenchmark)
{
    ...
    std::cout << "\n==================================================" << std::endl;
    std::cout << "           EKF PERFORMANCE BENCHMARK RESULTS" << std::endl;
    std::cout << "==================================================" << std::endl;
    std::cout << "Test Configuration:" << std::endl;
    std::cout << "├ Iterations per function: " << num_iterations << std::endl;
    std::cout << "├ Time step (dt): " << dt << " seconds" << std::endl;
    std::cout << "└ Measurement rate: Simulated high-quality odometry" << std::endl;
    std::cout << std::endl;

    std::cout << std::fixed << std::setprecision(4);
    std::cout << "Performance Results:" << std::endl;
    std::cout << "├ PREDICTION average time: " << avg_time_predict_us << " μs (" << avg_time_predict_us / 1000.0 << " ms)" << std::endl;
    std::cout << "├ CORRECTION average time: " << avg_time_correct_us << " μs (" << avg_time_correct_us / 1000.0 << " ms)" << std::endl;
    std::cout << "└ COMBINED cycle time: " << (avg_time_predict_us + avg_time_correct_us) << " μs (" << (avg_time_predict_us + avg_time_correct_us) / 1000.0 << " ms)" << std::endl;
    std::cout << std::endl;

    std::cout << "Real-Time Performance Analysis:" << std::endl;
    std::cout << "├ Maximum prediction rate: " << (1e6 / avg_time_predict_us) << " Hz" << std::endl;
    std::cout << "├ Maximum correction rate: " << (1e6 / avg_time_correct_us) << " Hz" << std::endl;
    std::cout << "└ Maximum combined rate: " << (1e6 / (avg_time_predict_us + avg_time_correct_us)) << " Hz" << std::endl;
    std::cout << "==================================================" << std::endl;
    std::cout << std::endl;
