...
#include <laser_msgs/msg/thrusts_motors.hpp>
...
{
  // Estrutura para guardar a mensagem mais recente de cada tipo
  ...
  private:
    ...
    void controlCallback(const laser_msgs::msg::ThrustsMotors::SharedPtr msg);
    void timerCallback();
    ...
    template <typename MsgT>
    typename MsgT::SharedPtr getSynchronizedMessage(const rclcpp::Time &ref_time, SensorDataBuffer<MsgT> &sensor_data);
    ...
    rclcpp::Subscription<laser_msgs::msg::ThrustsMotors>::SharedPtr control_sub_;
    ...
    // Buffers de dados para cada sensor
    ...
    SensorDataBuffer<laser_msgs::msg::ThrustsMotors> control_data_;
    ...
    Eigen::Matrix<double, 4, 1> last_control_input_;
